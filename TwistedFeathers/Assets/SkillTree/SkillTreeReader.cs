...
public class SkillTreeReader : MonoBehaviour {
    ...
    public bool CanSkillBeUnlocked(int id_skill)
    {
        ...
        {
            if(_skillInspected.level_req <= availablePoints) // Enough points available
            {
                int dependency = _skillInspected.pre_req;
                    if (_skills.TryGetValue(dependency, out _skillInspected))
                    {
    ...
    public bool UnlockSkill(int id_Skill)
    {
        if(_skills.TryGetValue(id_Skill, out _skillInspected))
        {
            if (_skillInspected.level_req <= availablePoints)
            {
                availablePoints -= _skillInspected.level_req;
