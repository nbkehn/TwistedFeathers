...
public class NodeBasedEditor : EditorWindow
{
    ...
    private void DrawPanel()
    {
        ...
        // Selected Field
        GUILayout.BeginHorizontal();
        EditorGUILayout.PrefixLabel(new GUIContent("Selected: ", "Specifies whether the particpant has this skill equipped"));
        selected.Selected = EditorGUILayout.Toggle(selected.Selected, GUILayout.Width(108));
        GUILayout.EndHorizontal();
    ...
    private void SaveSkillTree()
    {
        if (nodes.Count > 0)
        {
            ...
            for (int i = 0; i < nodes.Count; ++i)
            {
                ...
                skillTree.skilltree[i].Dependency = dependency;
            }
    ...
    private void LoadNodes()
    {
        ...
        if (File.Exists(nodePath))
        {
            ...
            if (File.Exists(skillPath))
            {
                ...
                for (int i = 0; i < nodes.Count; ++i)
                {
                        if (skillDictionary.TryGetValue(nodes[i].skill.Dependency, out outSkill))
                        {
                            for (int k = 0; k < nodes.Count; ++k)
                            {
    ...
    private void LoadSkillCreateNode(Skill skill, Vector2 position)
    {
        ...
        nodeCount = Mathf.Max(nodeCount, skill.ID) + 1;
    }
}